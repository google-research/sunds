...
def get_camera_rays(c_pos,
  ...
  if noisy:
    ...
    horizontal_offsets += (np.random.random(
        (height, width)) - 0.5) * pixel_width
    vertical_offsets += (np.random.random((height, width)) - 0.5) * pixel_height

  horizontal_offsets = (
      np.reshape(horizontal_offsets,
                 (height, width, 1)) * np.reshape(img_plane_horizontal,
                                                  (1, 1, 3)))
  vertical_offsets = (
      np.reshape(vertical_offsets,
                 (height, width, 1)) * np.reshape(img_plane_vertical,
                                                  (1, 1, 3)))
...
def depths_to_world_coords(depths,
                           rays,
                           camera_pos,
                           depth_noise=None,
                           noise_ratio=1.):
  ...
  if depth_noise is not None:
    noise_indicator = (np.random.random(depths.shape) <= noise_ratio).astype(
        np.float32)
    depths = depths + noise_indicator * np.random.random(
        depths.shape) * depth_noise
